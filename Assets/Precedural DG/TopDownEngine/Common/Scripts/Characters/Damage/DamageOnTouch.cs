...
namespace MoreMountains.TopDownEngine
{
    ...
    public class DamageOnTouch : MonoBehaviour
    {
        ...
        protected virtual void OnCollideWithDamageable(Health health)
        {
            ...
            _colliderHealth.Damage(TrueDamage(), gameObject, InvincibilityDuration, InvincibilityDuration, _damageDirection);
        ...
        private int TrueDamage() {
             int counterf = GameObject.Find("GameManager").GetComponent<GameManager>().Floor;
            if(this.gameObject.layer == 13 || this.gameObject.layer == 24) {
                return (DamageCaused * ((counterf / 10) + 1));
            }
			else if (this.gameObject.layer == 18) {
                double aux = DamageCaused * Atributos.Dest * 0.01;
                if (Random.Range(1, 101) <= Atributos.CritChance) return (int)(aux * Atributos.CritDamage);
                else return (int) aux;
            }
			else
		        return DamageCaused;	
