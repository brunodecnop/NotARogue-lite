...
public class LevelGeneration : MonoBehaviour {
    ...
    void CreateRooms() {
        ...
        for (int i = 0; i < numberOfRooms - 1; i++) {
            float randomPerc = ((float)i) / (((float)numberOfRooms - 2));
    ...
    void SetDoorsTP() {
        ...
        for (int x = 0; x < ((gridSizeX * 2)); x++) {
            for (int y = 0; y < ((gridSizeY * 2)); y++) {
                ...
                GameObject roomVar = GameObject.Find($"Room {rooms[x,y].gridPos.x},{rooms[x, y].gridPos.y}");
                


                if (rooms[x, y].doorTop != false) {
                    GameObject roomExitVar = GameObject.Find($"Room {rooms[x, y].gridPos.x},{rooms[x, y].gridPos.y + 1}");
                    GameObject doorVar = roomVar.transform.Find("DoorT(Clone)").gameObject;
                    GameObject doorExitVar = roomExitVar.transform.Find("DoorB(Clone)").gameObject;
                    

                    Teleporter tp;
                    Teleporter tpE;
                    MoreMountains.TopDownEngine.Room cr;
                    MoreMountains.TopDownEngine.Room tr;
                    tp = doorVar.GetComponent<Teleporter>();
                    tpE = doorExitVar.GetComponent<Teleporter>();
                    cr = roomVar.GetComponent<MoreMountains.TopDownEngine.Room>();
                    tr = roomExitVar.GetComponent<MoreMountains.TopDownEngine.Room>();
                    tp.Destination = tpE;
                    tp.CurrentRoom = cr;
                    tp.TargetRoom = tr;
                ...
                if (rooms[x, y].doorBot != false) {
                    GameObject roomExitVar = GameObject.Find($"Room {rooms[x, y].gridPos.x},{rooms[x, y].gridPos.y - 1}");
                    GameObject doorVar = roomVar.transform.Find("DoorB(Clone)").gameObject;
                    GameObject doorExitVar = roomExitVar.transform.Find("DoorT(Clone)").gameObject;
                ...
                if (rooms[x, y].doorRight != false) {
                    GameObject roomExitVar = GameObject.Find($"Room {rooms[x, y].gridPos.x + 1},{rooms[x, y].gridPos.y}");
                    GameObject doorVar = roomVar.transform.Find("DoorR(Clone)").gameObject;
                    GameObject doorExitVar = roomExitVar.transform.Find("DoorL(Clone)").gameObject;
                ...
                if (rooms[x, y].doorLeft != false) {
                    GameObject roomExitVar = GameObject.Find($"Room {rooms[x, y].gridPos.x -1},{rooms[x, y].gridPos.y}");
                    GameObject doorVar = roomVar.transform.Find("DoorL(Clone)").gameObject;
                    GameObject doorExitVar = roomExitVar.transform.Find("DoorR(Clone)").gameObject;
